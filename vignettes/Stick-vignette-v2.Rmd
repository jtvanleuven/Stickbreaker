...
To do this, we need to tell it where the model was saved and where the testing data was saved. Note this code block can get turned off by setting `fit.test.data` to FALSE. Note tha the multinomial model we employ here to assign model seleciton was generated above in the section titled, "b. Fit training data to multinomial model for model selection". Also notice that the resulting posteriors are output to the 'extdir' folder and given a name that is the same as the source data file with this appended: `n.muts, "muts_wPosts.csv"`.
...
We analyze two datasets in the code above (one in II. Basic Anlaysis and another in the first half of III. Detailed Features). Here we walk through analysis of another couple datasets to further illustrate the process.

## A. Mutation pair data from bacteriophage {-}
...
### 2. Simulate training data matching this dataset and fit multinomial {-}
...
```{r}
  ...
  simulate.training.data.from.priors <- FALSE
...
The results shows that, of the three models, stickbreaking is a far better approximation than additive or multiplictive. When only given these three options, the posterior probability of stickbrekaing is > 99.8%. This result also shows how the method can be used with double data.
...
deoptimization of synonymous codon usage in the capsid region, J. Virology 80(7):3259-3272. Note that the raw data are in plaque forming units per mL (PFU/mil). We make the log of this since when groth is exponential, growth rate should be proportional to the log of populaiton size.
